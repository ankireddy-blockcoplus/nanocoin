...
#if defined(NDEBUG)
# error "nanocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xec98f6d9320b7ab68426e9c632adbe95eb2cb904be6564b878c012a92bacfcc7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // nanocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "nanocoin Signed Message:\n";
...
{
    // nanocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // nanocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // nanocoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // nanocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // nanocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // nanocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe0e9d6fa4ea0ece4286e13296aaf87232d3286b05ced4465d1dee732fd2f3d38");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 07/frb/2019 Steve Jobs, Apple’s Visionary, Dies at 56";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04827aacd6336e15a8b0cdb02ebf1e8c5b0dab2558ed0aa885c92adfe2a96fac30d8b631e57b7fdbd1acadaf3e1f6dbe5cb2acbbd352d31c99045231a4c8780b1c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1549533252;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084688133;

        if (fTestNet)
        {
            block.nTime    = 1549533449;
            block.nNonce   = 386099064;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0c81581b9b3629bebf6da2a17d060564750539e69980c637d02ba1bc01172095"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xdc }; // nanocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("nanocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("nanocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static nanocoinMiner(CWallet *pwallet)
{
    printf("nanocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nanocoin-miner");
    ...
    try { loop {
        ...
        printf("Running nanocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("nanocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&nanocoinMiner, pwallet));
}
